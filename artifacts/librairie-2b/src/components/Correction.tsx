...
function Correction({ onNavigate }: CorrectionProps) {
  ...
  const groupOrdersByCustomer = (orders: Student[]) => {
    ...
    for (const order of orders) {
      ...
      if (!grouped[key]) {
        ...
        // Keep shared fields populated even when only some child rows store them
        if (grouped[key].avance == null && order.avance != null) {
          grouped[key].avance = order.avance
        }
        if (!grouped[key].note && order.note) {
          grouped[key].note = order.note
        }
        if (!grouped[key].email && order.email) {
          grouped[key].email = order.email
        }
        if (!grouped[key].telephone && order.telephone) {
          grouped[key].telephone = order.telephone
        }
  ...
  const buildEditForm = (order: GroupedOrder): EditFormData => ({
    nom: order.nom ?? '',
    ...
    email: order.email ?? '',
    telephone: order.telephone ?? '',
    avance: order.avance != null ? String(order.avance) : '',
    note: order.note ?? '',
    couverture_demandee: Boolean(order.couverture_demandee),
  })
  ...
  const handleEditOrder = async (order: GroupedOrder) => {
    // Prefill immediately so every input is visible with current list data
    setEditingOrder(order)
    setEditForm(buildEditForm(order))

    // Then reload the full order from DB so no child/field is missing
    try {
      const { data, error } = await supabase
        .from('students')
        .select('*')
      ...
      const matched = data.filter(
        row =>
          normalize(row.email) === normalize(order.email) &&
          normalize(row.telephone) === normalize(order.telephone)
      )
      const children = matched.length > 0 ? matched : data.filter(row =>
        order.children.some(c => c.id === row.id)
      )
      if (children.length === 0) return

      const fullOrder: GroupedOrder = {
        nom: order.nom,
        email: children.find(c => c.email)?.email ?? order.email,
        telephone: children.find(c => c.telephone)?.telephone ?? order.telephone,
        avance: children.find(c => c.avance != null)?.avance ?? order.avance,
        note: children.find(c => c.note)?.note ?? order.note,
        couverture_demandee: children.some(c => c.couverture_demandee) || order.couverture_demandee,
        created_at: children[0]?.created_at ?? order.created_at,
        children,
      }
      setEditingOrder(fullOrder)
      setEditForm(buildEditForm(fullOrder))
    } catch (err) {
      console.error('Erreur lors du chargement complet de la commande:', err)
    }
  }
  ...
  if (editingOrder && editForm) {
    return (
      <div className="max-w-4xl mx-auto px-3 sm:px-4 pb-12">
        <div className="mb-6 sm:mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-white rounded-2xl p-3 sm:p-4 shadow-sm border border-parchment-300">
          <button
            onClick={handleCancelEdit}
            className="self-start px-4 sm:px-5 py-2 bg-parchment-200 text-espresso-800 rounded-xl font-bold tracking-wide hover:bg-parchment-300 transition-colors uppercase text-xs"
          >
            ← Annuler
          </button>
          <h1 className="text-lg sm:text-xl font-heading font-bold text-espresso-900 truncate">
